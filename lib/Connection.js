...
Connection.prototype.write = function(data, callback) {
  ...
  if (this.state === Connection.DISCONNECTED) {
    return callback(new Error("Connection disconnected"));
  } else if (this.state === Connection.CONNECTING) {
    function connectingConnect() {
      that.removeListener('error', connectingError);
      doWrite();
    }
    function connectingError(err) {
      that.removeListener('connect', connectingConnect);
      callback(err);
    }
    this.once('connect', connectingConnect);
    this.once('error', connectingError);
  } else {
    doWrite();
  }
};
